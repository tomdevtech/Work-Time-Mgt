...
function ActivityManagement() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        if (selectedActivity) {
          await axios.patch(
            `http://localhost:3000/api/activities/${selectedActivity.code}`,
            values
          );
        } else {
          await axios.post('http://localhost:3000/api/activities', values);
        }
        fetchActivities();
        handleClose();
      } catch (error) {
  ...
  const handleDelete = async (code) => {
    try {
      await axios.delete(`http://localhost:3000/api/activities/${code}`);
      fetchActivities();
    } catch (error) {
